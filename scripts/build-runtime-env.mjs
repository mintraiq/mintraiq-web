...
const DEFAULT_PUBLIC_ENV = {
    /** Deploy environment: "production" | "preview" | "development". Empty for local builds. */
    envLabel: '',
...
// Vercel sets VERCEL_ENV on every build; PUBLIC_ENV_LABEL overrides it for non-Vercel hosts.
if (!merged.envLabel) {
    merged.envLabel = String(process.env.VERCEL_ENV ?? '').trim();
}
